function createRenderer(gl, resources) {
  ...
  function drawPrimitive(ctx, curScene, primitive, matrix, skinOrNull) {
    ...
    if (material) {
      ...
      if (shader.hasTransmission()) {
          ...
          if (t) {
              gl.uniform1i(programObj.uniformLocations.transmissionFramebuffer, OPAQUE_TEX_UNIT);  
              gl.uniform1f(programObj.uniformLocations.transmissionFactor, t.transmissionFactor || 0.0);
              gl.uniform2f(programObj.uniformLocations.viewportSize, ctx.width, ctx.height);
    
              gl.uniformMatrix4fv(programObj.uniformLocations.ViewMatrix, false, modelView);
              gl.uniformMatrix4fv(programObj.uniformLocations.ProjectionMatrix, false, perspective);
              gl.uniformMatrix4fv(programObj.uniformLocations.ModelMatrix, false, matrix);
        
              if (shader.hasTransmissionTexture() && t.transmissionTexture) {
    ...
    gl.uniformMatrix4fv(programObj.uniformLocations.Model, false, matrix);
  ...
  function drawNode(pass, ctx, scene, node, nodeID, nodeMatrix, parentModelMatrix) {
    ...
    if (node.mesh) {
      for (const prim of node.mesh.primitives) {
        ...
        // opaque pass에서는 transmission이 있는 프리미티브는 스킵
        if (pass === "opaque" && isTrans) continue;
        // transmission pass에서는 transmission이 없는 프리미티브는 스킵
  ...
  function render(ctx) {
    ...
    gl.activeTexture(gl.TEXTURE0 + OPAQUE_TEX_UNIT);
    gl.bindTexture(gl.TEXTURE_2D, opaqueColorTex);
    gl.generateMipmap(gl.TEXTURE_2D);

    for (const scene of ctx.scenes) {
      drawScene("transmission", ctx, scene);
    }

    
    // // 4. transparent texture to main
    // gl.bindFramebuffer(gl.FRAMEBUFFER, null);
    // gl.viewport(0, 0, gl.drawingBufferWidth, gl.drawingBufferHeight);
    // for (const scene of ctx.scenes) {
    //   drawScene("sdf", ctx, scene);
    // }
